...
from .types import ConfigFileData, FileInfoData, LogsData, LogsShareData, MotdData, RamData, ServerData
...
class Server:
    ...
    async def get_player_lists(self) -> list[str]:
        async with self._session.get(BASE_URL / "servers" / self.data.id / "playerlists") as reponse:
    ...
    async def get_player_list(self, list_name: str) -> list[str]:
        async with self._session.get(BASE_URL / "servers" / self.data.id / "playerlists" / list_name) as reponse:
            data: dict = await reponse.json()
    ...
    async def get_file_info(self, file_path: str) -> FileInfoData:
        async with self._session.get(BASE_URL / "servers" / self.data.id / "files" / "info" / file_path) as reponse:
            data: dict = await reponse.json()
    ...
    async def delete_file(self, file_path: str) -> str:
        async with self._session.delete(BASE_URL / "servers" / self.data.id / "files" / "data" / file_path) as reponse:
            data: dict = await reponse.json()

            return data["data"]

    async def get_config_file(self, file_path: str) -> list[ConfigFileData]:
        async with self._session.get(BASE_URL / "servers" / self.data.id / "files" / "config" / file_path) as reponse:
            data: dict = await reponse.json()
